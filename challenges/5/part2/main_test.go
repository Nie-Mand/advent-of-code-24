...
func TestSolve(t *testing.T) {
	t.Run("", func(t *testing.T) {
		...
		// part 2
		assert.Equal(t, 1, idx.failsAt([]int{61, 13, 29}), "Expected 1")
		assert.Equal(t, []int{61, 29, 13}, swap([]int{61, 13, 29}, 1), "Expected [61, 29, 13]")
		assert.Equal(t, []int{61, 29, 13}, idx.correct([]int{61, 13, 29}), "Expected [61, 29, 13]")
		assert.Equal(t, []int{61, 29, 13}, idx.correct([]int{61, 13, 29}), "Expected [61, 29, 13]")
		assert.Equal(t, true, idx.rightOrder(swap([]int{61, 13, 29}, 1)), "Expected true")
		assert.Equal(t, true, idx.rightOrder(idx.correct([]int{75, 97, 47, 61, 53})), "Expected true")
		assert.Equal(t, true, idx.rightOrder(idx.correct([]int{97, 13, 75, 29, 47})), "Expected true")

		assert.Equal(t, 123, solve(rules, updates), "Expected 123")
	})
}
